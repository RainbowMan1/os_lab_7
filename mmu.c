...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
  ...
  node_t* prev = current;
  while (current != NULL){
   int blk_size = current->blk->end - current->blk->start;
  ...
  current->next = NULL;
  current->blk->pid = pid;
  int original_end = current->blk->end;
  current->blk->end = current->blk->start+blocksize-1;
  list_add_ascending_by_address(alloclist, current->blk);
  
  block_t* fragment = (block_t *) malloc(sizeof(block_t));
  fragment->pid = 0;
  fragment->start = current->blk->end +1;
